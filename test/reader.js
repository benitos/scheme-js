...
describe('SchemeReader', function () {
    var reader = new SchemeReader();

    it('should have a `rawInput` property', function () {
        expect(reader).to.have.a.property('rawInput');
        expect(reader.rawInput).to.equal(null);
    });
    ...
    describe('.readCons()', function () {
        ...
        it('should throw an error if there is something wrong with the syntax', function () {
            var parser = new SchemeParser('(display "hi"');

            expect(reader.readCons.bind(reader, parser)).to.throw('Syntax error - premature end of string (`(display "hi"`)!');
        });
    });
