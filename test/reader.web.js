...
var SchemeReaderWeb = require('../src/reader.web');
...
describe('SchemeReaderWeb', function () {
    var inputMock = {
    ...
    var reader = new SchemeReaderWeb(inputMock);

    it('should have a `rawInput` property', function (done) {
        expect(reader).to.have.a.property('rawInput');
        expect(reader.rawInput).to.be.an('object');
        expect(reader.rawInput).to.have.a.property('addEventListener');
        expect(reader.rawInput.addEventListener).to.be.a('function');
        expect(reader.rawInput).to.have.a.property('data');
        expect(reader.rawInput.data).to.be.a('function');

        reader.on('print', function (payload) {
            expect(payload).to.equal(evtMock.detail.replace(/\n/g, '\n&raquo; '));
            done();
        });

        reader.rawInput.data(evtMock);
    });
